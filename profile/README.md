...
**풀문(Fullmoon) 네트워크의 공개 개발 생태계.**

하나의 PostgreSQL 원장을 여러 봇·대시보드·클라이언트가 함께 읽는 구조를
오픈소스로 공개한다. 각자 화폐를 만들지 않고, 같은 원장을 같은 정의로 읽는 것 —
그게 이 조직의 전부다.
...
시작 가이드는 [fullmoon-docs](https://github.com/Fullmoon-OSS/fullmoon-docs)에서.

## 통합 등록

SDK·API 위에 만든 봇·대시보드·도구는
[INTEGRATIONS.md](https://github.com/Fullmoon-OSS/fullmoon-sdk/blob/main/INTEGRATIONS.md)
카탈로그에 PR 한 장으로 등록할 수 있다.

## 라이선스

SDK·API·문서는 MIT, fullmoon-client는 GPL-3.0.
